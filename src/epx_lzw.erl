...
%% reverse bits in a by
%% magic code from: http://graphics.stanford.edu/~seander/bithacks.htm
rbits8(Code) ->
    ((Code*16#0202020202) band 16#010884422010) rem 1023.
...
comp1(<<>>,_CC,_Stripe,Omega,BitLen,_Count,Z,Build) ->
    ...
    Bin = if PaddL =:= 8 ->
		  buildbin(lists:reverse(Codes));
	     true ->
		  buildbin(lists:reverse([{PaddL, 0}|Codes]))
	  end,
    {Z#z.startlen-1, list_to_binary(lists:reverse([Bin|Acc]))};
...
write(Code, CLen, {Totlen, List, Acc}) ->
    ...
    if 
        NewLen rem 8 =:= 0 ->
            case buildbin(lists:reverse([{CLen,Code}|List])) of
        ...
        NewLen > 100 ->
            {Bin,NewList} = buildbin(lists:reverse([{CLen,Code}|List])),
	    Sum = sumlen(NewList),
	    {Sum,lists:reverse(NewList),[Bin|Acc]};
