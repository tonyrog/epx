 ...
 ***************************************************************************/
/*
 * ALTIVEC support (Incomplete - not used for a while)
 *
 */

#include "epx_simd_int.h"
#include "epx_simd_altivec.h"
...
void epx_simd_fill_32_altivec(uint8_t* dst, uint32_t v, size_t n)
{
...
void epx_simd_fill_area_blend_rgb24_altivec(uint8_t* dst,int dst_wb,
					    epx_pixel_t p,
					    unsigned int width,
					    unsigned int height)
{
    /* Maybe use one rotating register ? instead of 3 ... */
    epx_vector_i8_t s8_0 = epx_simd_vector_Set8(p.r,p.g,p.b,p.r,p.g,p.b,p.r,p.g,
					   p.b,p.r,p.g,p.b,p.r,p.g,p.b,p.r);
    epx_vector_i8_t s8_1 = epx_simd_vector_Set8(p.g,p.b,p.r,p.g,p.b,p.r,p.g,p.b,
					   p.r,p.g,p.b,p.r,p.g,p.b,p.r,p.g);
    epx_vector_i8_t s8_2 = epx_simd_vector_Set8(p.b,p.r,p.g,p.b,p.r,p.g,p.b,p.r,
	...
	while(wb1 >= EPX_SIMD_VECTOR_SIZE) {
	    ...
	    switch(offs1) {
	    case 0: td = epx_simd_alpha_32_altivec(a8,s8_0,td); break;
	    case 1: td = epx_simd_alpha_32_altivec(a8,s8_1,td); break;
	    case 2: td = epx_simd_alpha_32_altivec(a8,s8_2,td); break;
	    default: return;
	    }
